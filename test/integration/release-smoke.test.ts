...
async function npmPack(destDir: string): Promise<string> {
  const { stdout } = await execFileAsync('npm', ['pack', '--pack-destination', destDir], {
    cwd: ROOT,
  });
...
describe('release smoke test', () => {
  ...
  it('SHA256SUMS content matches actual tarball checksum', async () => {
    ...
    try {
      ...
      const { stdout: sha256sumOutput } = await execFileAsync('sha256sum', [tarball]);
      const expectedLine = `${sha256sumOutput.trim().split(/\s+/)[0]}  ${tarballName}`;

      const tarballBytes = await readFile(tarball);
      const computedHash = createHash('sha256').update(tarballBytes).digest('hex');
