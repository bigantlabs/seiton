...
describe("analyzeItems", () => {
		...
		it("does not group items with different URIs", () => {
			const items = [
				...
				makeItem({
					id: "2",
					login: {
						uris: [{ match: null, uri: "https://b.com" }],
						username: "u",
						password: "Str0ng!Passw0rd",
						totp: null,
					},
		...
		it("suggests folders for unassigned items matching rules", () => {
			const items = [
				makeItem({
					...
					name: "Chase Bank",
					login: {
						uris: [{ match: null, uri: "https://chase.com" }],
						username: "u",
						password: "Str0ng!Passw0rd",
						totp: null,
					},
		...
		it("suggests folders for items in folders when preserve_existing is false", () => {
			...
			const config = makeConfig({
				folders: {
					...
					enabled_categories: ["Banking & Finance"],
					custom_rules: [],
				},
			});
	...
	describe("folder finding producer shape", () => {
		...
		it("produces FolderFinding via custom_rules through analyzeItems", () => {
			const item = makeItem({
				...
				login: {
					uris: [{ match: null, uri: "https://exchange.example.com" }],
					username: "trader",
					password: "Str0ng!Passw0rd",
					totp: null,
				},
		...
		it("classifies items by URI when name does not match", () => {
			const item = makeItem({
				...
				name: "My Account",
				login: {
					uris: [{ match: null, uri: "https://gmail.com/login" }],
					username: "u",
					password: "Str0ng!Passw0rd",
					totp: null,
				},
		...
		it("does not produce FolderFinding when no rule matches", () => {
			const item = makeItem({
				...
				name: "Random Service",
				login: {
					uris: [{ match: null, uri: "https://uniquesite.example.com" }],
					username: "u",
					password: "Str0ng!Passw0rd",
					totp: null,
				},
		...
		it("filters null URIs before classification", () => {
			const item = makeItem({
				...
				login: {
					uris: [
						{ match: null, uri: null },
						{ match: null, uri: "https://netflix.com" },
					],
					username: "u",
					password: "Str0ng!Passw0rd",
					totp: null,
	...
	describe("determinism", () => {
		...
		it("produces equivalent findings regardless of input order", () => {
			...
			const normalizeGroupOrder = (findings: typeof forward) =>
				sortFindings(findings).map((f) => {
					if ("items" in f) {
						return {
							...f,
							items: [...f.items].sort((a, b) => a.id.localeCompare(b.id)),
						};
					}
					return f;
				});
			...
			assert.deepStrictEqual(
				normalizeGroupOrder(forward),
				normalizeGroupOrder(reversed),
			);
		});
	});
	...
	describe("compare_only_primary_uri: false", () => {
		...
		it("groups items with identical URIs in different order when compare_only_primary_uri is false", () => {
			...
			assert.equal(
				dupes.length,
				1,
				"identical URIs in different order match when compare_only_primary_uri is false",
			);
		});
	});
	...
	describe("null URI entries", () => {
		it("handles login.uris with uri: null without crashing", () => {
			...
			assert.equal(
				missing.length,
				1,
				"item with only null URIs should flag missing uri",
			);
		});
	...
	describe("colon-key early-continue guard", () => {
		it('skips items with null URI and null username (key would be ":")', () => {
			...
			assert.equal(
				dupes.length,
				0,
				"items with empty domain and empty username should not be grouped as duplicates",
			);
		});
		...
		it('skips items with no uris and empty username (key would be ":")', () => {
			...
			assert.equal(
				dupes.length,
				0,
				"items with empty dedup key should not be grouped as duplicates",
			);
		});
	});
	...
	describe("extra_common_passwords", () => {
		it("flags password containing an extra_common_passwords entry", () => {
			...
			assert.ok(
				weak.length > 0,
				"password containing extra common substring should be flagged at zxcvbn_min_score: 4",
			);
			if (weak[0]!.category === "weak") {
				assert.ok(
					weak[0].score < 4,
					`score should remain below configured minimum, got ${weak[0].score}`,
				);
			}
		});
		...
		it("does not flag same password when extra_common_passwords is empty", () => {
			...
			assert.equal(
				weak.length,
				0,
				"same password should score 4 (pass) when extra_common_passwords is empty",
			);
		});
	});
	...
	describe("zxcvbn integration", () => {
		it("flags Password1! which passes heuristic checks but is weak by dictionary analysis", () => {
			...
			assert.ok(
				weak.length > 0,
				"Password1! should be flagged by zxcvbn despite passing heuristic checks",
			);
			if (weak[0]!.category === "weak") {
				assert.ok(
					weak[0].score <= 1,
					`Password1! should remain low-score under zxcvbn, got ${weak[0].score}`,
				);
			}
		});
		...
		it("provides zxcvbn-style feedback strings in reasons", () => {
			...
			if (weak[0]!.category === "weak") {
				assert.ok(
					weak[0].reasons.some((r) =>
						/(common|used password|heavily used)/i.test(r),
					),
					`zxcvbn feedback should indicate password reuse/commonness, got: ${JSON.stringify(weak[0].reasons)}`,
				);
			}
		});
	});
		...
		it("finds near-duplicate items when threshold is enabled", () => {
			const items = [
				...
				makeItem({
					...
					name: "GitHubb",
					login: {
						uris: [{ match: null, uri: "https://githubb.com" }],
						username: "u2",
						password: "An0ther!Pass99",
						totp: null,
					},
