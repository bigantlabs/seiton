...
describe('collectOpsFromFindings', () => {
  ...
  it('creates folder ops for folder findings', () => {
    ...
    const findings: Finding[] = [
      { category: 'folders', item, suggestedFolder: 'Banking', matchedRule: 'bank' },
    ];
    const result = collectOpsFromFindings(findings, {
      skipCategories: [],
      limitPerCategory: null,
  ...
  it('deduplicates create_folder ops for same folder', () => {
    const findings: Finding[] = [
      { category: 'folders', item: makeItem({ id: '1' }), suggestedFolder: 'Banking', matchedRule: 'bank' },
      { category: 'folders', item: makeItem({ id: '2' }), suggestedFolder: 'Banking', matchedRule: 'bank' },
    ];
    const result = collectOpsFromFindings(findings, {
      skipCategories: [],
      limitPerCategory: null,
