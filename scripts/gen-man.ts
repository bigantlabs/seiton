...
const EXIT_CODE_DESCRIPTIONS: Record<number, string> = {
	...
	[ExitCode.USAGE]:
		"Invalid arguments, unknown command, or non-interactive terminal.",
	...
	[ExitCode.USER_INTERRUPT]:
		"User interrupt (SIGINT). Pending ops saved if enabled.",
};

function getLastCommitDate(): string {
	try {
		return execFileSync("git", ["log", "-1", "--no-merges", "--format=%cs"], {
			encoding: "utf-8",
		}).trim();
	} catch {
		return "1970-01-01";
...
function buildCommandsSection(): string {
	const cmds = [
		[
			"audit",
			"Fetch, analyze, review findings, apply approved changes (default).",
		],
...
function buildEnvironmentSection(): string {
	const vars = [
		[
			"BW_SESSION",
			"Bitwarden session token. Required for audit, resume, and report.",
		],
